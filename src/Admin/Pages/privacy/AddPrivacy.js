...
const Contant = () => {
  ...
  const bindData = async () => {
    await apiAdminConfig.get(`/api/auth/master/page/edit/privacy-policy`).then((response) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setErrors }) => {
      ...
      await apiAdminConfig
        .post("/api/auth/master/page/update/privacy-policy", formData, { setErrors })
        .then((response) => {
          if (response?.status === 200) {
            snackbar({
        ...
        <>
          <Row className="mt-4">
            <Col md={6} style={{ textAlign: "left" }}>
              <h3 style={{ height: "40px" }}>Add Privacy Policy</h3>
            </Col>
            <Col md={4}></Col>
